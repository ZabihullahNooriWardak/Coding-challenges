...
bool isAnagram(String str1, String str2) {
  ...
  for (int i = 0; i < str1.length; i++) {
    ...
    character = str1[i];
    for (int j = 0; j < str1.length; j++) {
      if (character == str1[j] && i != j) {
        occurenceCounter += 1;
      }
    }
    bool result = anotherFunc(occurenceCounter, character, str2);
    if (result == false) {
      return result;
    }
  }
...
bool anotherFunc(int numberOfOccurence, String character, String second) {
  int counter = 0;
  for (int i = 0; i < second.length; i++) {
    if (second[i] == character) {
      counter++;
    }
  }
  if (counter == numberOfOccurence) {
    return true;
  }
  return false;
}
